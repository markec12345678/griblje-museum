 ...
 *                        95/95 zapisov, 429 vrstic virov, 331 identitet,
...
if (exN === 95) ok("95/95 zapisov"); else err(`zapisov: ${exN}`);
if (mvgN === 95) ok("95/95 muzejskih številk"); else err(`MVG: ${mvgN}`);
if (srcN === 429) ok("429 vrstic virov"); else err(`vrstic virov: ${srcN}`);
if (identities === 331) ok("331 identitet virov (A1/A2)"); else err(`identitet: ${identities}`);
