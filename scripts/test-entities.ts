 ...
 *   T9  HTTP regresija             — 112/112 strani, 112/112 IIIF, OpenData 565,
 *                                    QR, sprehod, sitemap 98
 *
...
section("T4 — Evidence/source vezi (veljavnost + točkovne vsebinske preverbe)");
{
  ...
  check(phases === 375 && broken === 0, `T4.4 biografije: 375 faz, 0 zdrobljenih sourceIndex (${phases}/${broken})`);
}
...
section("T5 — Ločitev identitet oseb (brez napačne avtomatske združitve)");
{
  ...
  // 36. val: dodatni dve vrstici polnega PDF-a SEM (MVG-004 + MVG-043, en sourceKey — deljen).
  // B-kandidat TASK 38 (dve vrstici Etnologa z ločenima sourceKey) ostaja kustosu.
  check(sopek.length === 5 && sopekKeys.size === 4,
    `T5.12 vir »Šopek poljskih cvetlic«: 2 vrstici Etnologa (MVG-010/MVG-043) + 1 prepis (MVG-043) + 2 vrstici PDF SEM (MVG-004/MVG-043, deljen) = 5 VRSTIC, 4 LOČENA sourceKey (${sopek.length}/${sopekKeys.size}) — B-kandidat TASK 38 ostaja kustosu`);
...
section("T7 — Ločitev identitet dogodkov");
{
  ...
  check(bySlug.size === 113, "T7.5 zapisa MVG-014 in MVG-056 ostajata LOČENA zapisa (113/113)");
...
{
  // Zbirka sama.
  check(seedExhibits.length === 113, `T8.1 113/113 zapisov (${seedExhibits.length})`);
  check(seedExhibits.filter((e) => /^MVG-\d{3}$/.test(e.museumNo ?? "")).length === 113, "T8.2 113/113 muzejskih številk MVG");
  ...
  check(SOURCE_USAGE.size === 453, `T8.6 453 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 572, `T8.11 572 vrstic virov (${srcRows})`);

  // Biografije.
  let phases = 0;
  for (const b of OBJECT_BIOGRAPHIES) phases += b.phases.length;
  check(phases === 375 && OBJECT_BIOGRAPHIES.length === 94, `T8.12 94 biografij, 375 faz (${OBJECT_BIOGRAPHIES.length}/${phases})`);
}
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(okPages === 113, `T9.1 113/113 objektnih strani (${okPages}; ${badPages.join(",") || "vse 200"})`);
    ...
    check(okManifests === 113 && withSources === 113, `T9.2 113/113 IIIF manifestov z viri (${okManifests} manifestov, ${withSources} z ≥1 virom)`);
    ...
    check(od.counts?.exhibits === 113 && od.counts?.sources === 572, `T9.3 OpenData: 113 zapisov / 572 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 572 && totalRows === 572, `T9.4 OpenData sourceKey 572/572 (${withKey}/${totalRows})`);
    ...
    check(locs === 114, `T9.8 sitemap: 114 URL (${locs})`);
  }
}
