 ...
 *   T9  HTTP regresija             — 112/112 strani, 112/112 IIIF, OpenData 561,
...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 443, `T8.6 443 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 561, `T8.11 561 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 112 && od.counts?.sources === 561, `T9.3 OpenData: 112 zapisov / 561 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 561 && totalRows === 561, `T9.4 OpenData sourceKey 561/561 (${withKey}/${totalRows})`);
