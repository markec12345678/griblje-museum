 ...
 *   T9  HTTP regresija             — 96/96 strani, 96/96 IIIF, OpenData 435,
...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 336, `T8.6 336 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 435, `T8.11 435 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 96 && od.counts?.sources === 435, `T9.3 OpenData: 96 zapisov / 435 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 435 && totalRows === 435, `T9.4 OpenData sourceKey 435/435 (${withKey}/${totalRows})`);
