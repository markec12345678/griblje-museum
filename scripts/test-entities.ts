 ...
 *   T9  HTTP regresija             — 96/96 strani, 96/96 IIIF, OpenData 512,
...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 402, `T8.6 402 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 512, `T8.11 512 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 101 && od.counts?.sources === 512, `T9.3 OpenData: 101 zapisov / 512 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 512 && totalRows === 512, `T9.4 OpenData sourceKey 512/512 (${withKey}/${totalRows})`);
