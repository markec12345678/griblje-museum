...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 452, `T8.6 452 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 571, `T8.11 571 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 113 && od.counts?.sources === 571, `T9.3 OpenData: 113 zapisov / 571 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 571 && totalRows === 571, `T9.4 OpenData sourceKey 571/571 (${withKey}/${totalRows})`);
