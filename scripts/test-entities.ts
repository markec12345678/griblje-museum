 ...
 *   T9  HTTP regresija             — 112/112 strani, 112/112 IIIF, OpenData 564,
...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 446, `T8.6 446 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 564, `T8.11 564 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 112 && od.counts?.sources === 564, `T9.3 OpenData: 112 zapisov / 564 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 564 && totalRows === 564, `T9.4 OpenData sourceKey 564/564 (${withKey}/${totalRows})`);
