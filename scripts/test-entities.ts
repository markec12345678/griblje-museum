 ...
 *   T9  HTTP regresija             — 111/111 strani, 111/111 IIIF, OpenData 556,
...
section("T8 — Obstoječe relacije objektov nespremenjene (walks/related/sourceIndex/sourceKey/usedBy)");
{
  ...
  check(SOURCE_USAGE.size === 438, `T8.6 438 identitet virov (${SOURCE_USAGE.size})`);
  ...
  check(srcRows === 556, `T8.11 556 vrstic virov (${srcRows})`);
...
section("T9 — HTTP regresija (96/96 strani, 96/96 IIIF, OpenData, QR, sitemap)");
{
  ...
  if (!serverUp) {
    ...
    check(od.counts?.exhibits === 111 && od.counts?.sources === 556, `T9.3 OpenData: 111 zapisov / 556 virov (${od.counts?.exhibits}/${od.counts?.sources})`);
    check(withKey === 556 && totalRows === 556, `T9.4 OpenData sourceKey 556/556 (${withKey}/${totalRows})`);
