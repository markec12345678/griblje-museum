...
section("T7 — PODATKOVNA REGRESIJA (invariante osnovne linije)");
{
  ...
  check(rows === 588, "T7.2 588 vrstic virov", `=${rows}`);
  check(SOURCE_USAGE.size === 469, "T7.3 469 identitet virov (40. val: +2 — laj-2024-rul-diploma, belokranjec-2026-veselic-zborovodkinja)", `=${SOURCE_USAGE.size}`);
...
section("T8 — HTTP REGRESIJA (živ strežnik :3000)");
{
  ...
  if (!serverUp) {
    ...
    check(
      od.counts?.exhibits === 113 && od.counts?.sources === 588,
      "T8.3 OpenData: 113 zapisov / 588 virov",
      `${od.counts?.exhibits}/${od.counts?.sources}`
    );
    check(withKey === 588 && totalRows === 588, "T8.4 OpenData sourceKey 588/588", `${withKey}/${totalRows}`);
