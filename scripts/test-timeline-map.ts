...
section("T7 — PODATKOVNA REGRESIJA (invariante osnovne linije)");
{
  ...
  check(rows === 582, "T7.2 582 vrstic virov", `=${rows}`);
  check(SOURCE_USAGE.size === 463, "T7.3 463 identitet virov (40. val: +2 — laj-2024-rul-diploma, belokranjec-2026-veselic-zborovodkinja)", `=${SOURCE_USAGE.size}`);
...
section("T8 — HTTP REGRESIJA (živ strežnik :3000)");
{
  ...
  if (!serverUp) {
    ...
    check(
      od.counts?.exhibits === 113 && od.counts?.sources === 582,
      "T8.3 OpenData: 113 zapisov / 582 virov",
      `${od.counts?.exhibits}/${od.counts?.sources}`
    );
    check(withKey === 582 && totalRows === 582, "T8.4 OpenData sourceKey 582/582", `${withKey}/${totalRows}`);
