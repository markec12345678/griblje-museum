...
export const ENTITY_QUEUE: EntityQueueItem[] = [
  ...
  },
  {
    ...
    priority: "P3",
    questionSi:
      "PUA p48/p49 (1825): 51. val (issue #35) je zaključek prebral: Nro. 96 = »Wiese[?]/Hiesige in der Gemeinde Grübln« (dolgi seznami parcel po sekcijah — skupna/nerazdeljena zemlja?), Nro. 97 = »Philipp De Giammo Zucchelli« (2×; branje »F. Kirch« opuščeno), parcela 2700, zaključna formula »Ich Amtl[ich] am 10. Jänner 1825« (MESEC PREBRAN; 56. val @ nativna ločljivost dodaja »um 10. Januar 1825« — 2 vira, mesec POTRJEN) s tremi podpisi (Mumppen[?], Hollmayr[?], 1 nečitljiv; 56. val vidi še »Bürgermeister«). REŠENO (56. val): p26/p42 ponovno prenešena kot PDF-native popolni strani (1391×2145 / 1400×2151) — p26 = glava + začetek vpisa 52, p42 = vpisa 85/86 (aritmetika val 48 potrjena). Ostaja: identiteta Zucchellija (italijanska oblika? Hutmayer[?]?) in natančna transkripcija zaključnih seznamov.",
    questionEn:
      "PUA p48/p49 (1825): val 51 (issue #35) read the closing: no. 96 = 'Wiese[?]/Hiesige in der Gemeinde Grübln' (long section-by-section parcel lists — common/undivided land?), no. 97 = 'Philipp De Giammo Zucchelli' (2×; the 'F. Kirch' reading abandoned), parcel 2700, the closing formula 'Ich Amtl[ich] am 10. Jänner 1825' (THE MONTH READ; val 56 at native resolution adds 'um 10. Januar 1825' — 2 sources, the month CONFIRMED) with three signatures (Mumppen[?], Hollmayr[?], 1 illegible; val 56 also sees 'Bürgermeister'). RESOLVED (val 56): p26/p42 re-downloaded as complete PDF-native pages (1391×2145 / 1400×2151) — p26 = header + start of entry 52, p42 = entries 85/86 (val 48 arithmetic confirmed). Remaining: the identity of Zucchelli (an Italian form? Hutmayer[?]?) and an exact transcription of the closing lists.",
    slugs: ["griblje-vas"],
  },
  {
    id: "P2-E13",
  ...
  },
  {
    ...
    priority: "P3",
    questionSi:
      "PT N083 — Protocoll der Bau Parcellen (1825, docid 41781; 53./54. val): popoln konsenzni register 100 vrstic / 97 BP (40 STABLE, 51 REVIEW-CONFLICT, 9 REVIEW) + usklajevanje (54. val): PT lastniška imena NEZANESLJIVA (0/39 imenskih soglasij z PUA; isti bp med prehodi dobi 3–4 različna imena — slab sken) → PUA = edina lastniška avtoriteta, PT uporaben le strukturno; p7-rep zoom branje ima +1 pomik vrstic; vrzel BP 15–20; Musterstellung der Gebäude na p8 (obseg nejasen); polovične parcele 7½/8½ na p3; Kurrent 3↔5 (PT 46/48→»h.63« verjetno 65) in 2↔3 (PUA h.29 opomba »B.P. 95. 97.« vs PT 95→h.39/40). Ostaja: kontrole v polni ločljivosti ob delujočem arhiv.si.",
    questionEn:
      "PT N083 — Protocoll der Bau Parcellen (1825, docid 41781; vals 53/54): full consensus register of 100 rows / 97 building parcels (40 STABLE, 51 REVIEW-CONFLICT, 9 REVIEW) + reconciliation (val 54): PT owner names UNRELIABLE (0/39 name agreements with PUA; the same bp gets 3–4 different names between passes — poor scan) → PUA is the sole owner authority, PT usable structurally only; the p7-tail zoom read has a +1 row drift; the BP 15–20 gap; Musterstellung der Gebäude on p8 (extent unclear); half parcels 7½/8½ on p3; Kurrent 3↔5 (PT 46/48→'h.63' probably 65) and 2↔3 (PUA h.29 note 'B.P. 95. 97.' vs PT 95→h.39/40). Remaining: full-resolution controls once arhiv.si is reachable.",
    slugs: ["griblje-vas"],
  },
  {
    id: "P2-E15",
    priority: "P2",
    questionSi:
      "Zemljevid vasi 1825 — revizija 11 vezav lastnikov (54. val) + kontrola v polni ločljivosti (56. val, PDF-native + pasovni izrezi 300 dpi): PRELOM — PUA no. 95 (p47, h.70) Zollamt IMA opombo »B.P. 98.« (v nadaljevalni vrstici vpisa; 2× branja; stara branja je izpustila) → lastniška vezava bp 98 = k.k. Zollamt VERIFIED-2x (PUA opomba + PT bp 98 »A. Zollamt«); hišna št. h.70 (PUA) vs h.20 (PT) → hišna vezava ostaja REVIEW-CONFLICT. PUA p18 no. 35 »b. P. 56. 38.« znak-po-znak POTRJENO (bp 86 ostaja REVIEW-CONFLICT, utemeljen); PUA p30 no. 59 »19. Pp.« POTRJENO (bp 92 ostaja REVIEW); PT p4 24/25 → h.1 potrjeno; PUA p6: opomba »B.P. 90.« obstaja (3. potrditev), ampak vezava na Nro. 7 vs 8 ostaja neusklajena (pomik vrstic med prehodi; h.43/h.63) → bp 90 ostaja REVIEW; PT p5 46-48/76: 3-kratno nesoglasje (63/65/62; 68/51/28 — Kurrent 2↔3↔5↔6) → ostaja REVIEW-CONFLICT. Ostaja: bp 90 vezava Nro. 7/8, PT p5 Kurrent, identiteta »Merial« v imenu Zollamta.",
    questionEn:
      "Village map 1825 — audit of 11 owner links (val 54) + full-resolution control (val 56, PDF-native + 300 dpi band crops): BREAKTHROUGH — PUA no. 95 (p47, h.70) Zollamt HAS the note 'B.P. 98.' (in the entry's continuation line; 2× reads; older readings missed it) → owner binding bp 98 = k.k. Zollamt VERIFIED-2x (PUA note + PT bp 98 'A. Zollamt'); house no. h.70 (PUA) vs h.20 (PT) → the house link remains REVIEW-CONFLICT. PUA p18 no. 35 'b. P. 56. 38.' CONFIRMED character-by-character (bp 86 stays REVIEW-CONFLICT, justified); PUA p30 no. 59 '19. Pp.' CONFIRMED (bp 92 stays REVIEW); PT p4 24/25 → h.1 confirmed; PUA p6: the note 'B.P. 90.' exists (3rd confirmation), but its anchoring to Nro. 7 vs 8 remains unresolved (row drift between passes; h.43/h.63) → bp 90 stays REVIEW; PT p5 46-48/76: three-way disagreement (63/65/62; 68/51/28 — Kurrent 2↔3↔5↔6) → remains REVIEW-CONFLICT. Remaining: bp 90 anchoring Nro. 7/8, PT p5 Kurrent, the identity of 'Merial' in the Zollamt name.",
    slugs: ["griblje-vas"],
  },
];
