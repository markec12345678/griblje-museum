...
const LAYERS = new Set(["all", "map_objects", "houses", "toponyms", "sheets"]);

/**
 * GET /api/atlas/map — podatkovni zemljevid (issue #42 §15, PASS 5).
 *
 * ?layer=all (privzeto) | map_objects | houses | toponyms | sheets
 * ?sheet=A01..A05 — filter map_objects po katastrskem listu
 *
 * Vsak objekt nosi node_id + evidence_url (sledljivost do vira).
 * Nič ne ugiba: koordinate samo kjer jih graf že ima (A01 PROVIZORIČNO);
 * parcele brez geometrije (§9); hiša brez dokazane BP↔MO veze = NOT_LOCATED.
 */
